...
args = argparse.Namespace(mri_sequences='ABDEFKT', data_sequences='TBAKDEF', train_dir='tbakd3_npy/5folds/4/train', valid_dir='tbakd3_npy/5folds/4/valid', embed_dim=10, pooling=True, merge_seq=True)
...
path = model.hparams.valid_dir.replace('valid', 'embs/dwac_valid_emb10.merged.pkl')
pickle.dump((val_fids, inputs, labels, embeds), open(path, "wb"))
print("Encoded valid embeddings (fids, inputs, labels, embeds) at " + path)

path = model.hparams.train_dir.replace('train', 'embs/dwac_train_emb10.merged.pkl')
